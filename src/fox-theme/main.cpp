// fox-theme — namespace dispatcher for theme tools.
// Mirrors fox-sec/main.cpp.
...
void print_help(const std::vector<fox_common::dispatch::Entry>& registry) {
    fox_common::dispatch::print_help(
        "fox-theme — theme management.\n"
        "Usage: fox theme <subcommand> [args]    (or directly: fox-theme <subcommand>)",
        registry
    );
    std::cout << "\nRun 'fox theme <subcommand> --help' for tool-specific help.\n";
}
...
int main(int argc, char** argv) {
    ...
    if (is_version_arg(argv[1])) { std::cout << "fox-theme " << kVersion << "\n"; return 0; }

    const std::string sub = argv[1];
    const auto* entry = fox_common::dispatch::find(registry, sub);
    if (!entry) {
        std::cerr << "fox-theme: unknown subcommand '" << sub << "'.\n"
                  << "Run 'fox theme help' to see available subcommands.\n";
        return 2;
    }
