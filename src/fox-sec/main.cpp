// fox-sec — namespace dispatcher for security tools.
//
// Invoked by `fox sec <subcommand> [args]` after the top-level fox
// binary recognizes the `sec` namespace. Mirrors fox-ai/main.cpp
// modulo the registry source and naming.
//
// Also runnable directly as `fox-sec <subcommand>` for script use.
...
constexpr const char* kVersion = "0.1.0 (Phase 3)";
...
void print_help(const std::vector<fox_common::dispatch::Entry>& registry) {
    fox_common::dispatch::print_help(
        "fox-sec — security tools.\n"
        "Usage: fox sec <subcommand> [args]    (or directly: fox-sec <subcommand>)",
        registry
    );
    std::cout << "\nRun 'fox sec <subcommand> --help' for tool-specific help.\n";
}
...
int main(int argc, char** argv) {
    ...
    if (!entry) {
        std::cerr << "fox-sec: unknown subcommand '" << sub << "'.\n"
                  << "Run 'fox sec help' to see available subcommands.\n";
        return 2;
    }
