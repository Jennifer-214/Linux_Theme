// fox-sys — namespace dispatcher for system tools.
...
void print_help(const std::vector<fox_common::dispatch::Entry>& registry) {
    fox_common::dispatch::print_help(
        "fox-sys — system tools.\n"
        "Usage: fox sys <subcommand> [args]    (or directly: fox-sys <subcommand>)",
        registry
    );
    std::cout << "\nRun 'fox sys <subcommand> --help' for tool-specific help.\n";
}
...
int main(int argc, char** argv) {
    ...
    if (is_version_arg(argv[1])) { std::cout << "fox-sys " << kVersion << "\n"; return 0; }

    const std::string sub = argv[1];
    const auto* entry = fox_common::dispatch::find(registry, sub);
    if (!entry) {
        std::cerr << "fox-sys: unknown subcommand '" << sub << "'.\n"
                  << "Run 'fox sys help' to see available subcommands.\n";
        return 2;
    }
