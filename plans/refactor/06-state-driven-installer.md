...
- **Polish round + bug fixes (shipped 2026-05-19):** 8 more state_checks (ufw, endlessh, greetd, papirus_icons, catppuccin_cursor, gpg_agent_cache, keyring_full, noexec_tmp) — coverage now 14 / 52 modules. **Bug**: dispatcher used to stamp `source_hash=""` for every module, which meant hash-based state_checks re-classified as Conflict on every subsequent install. Fixed by hashing the sentinel post-run via `wizard::conflict_sentinel`. **Bug**: `--resume` and `--phase` had been parsed by `args::parse` since the bash port but were never read in `main()`. Both wired up.
- **Cutover (shipped 2026-05-19, commit `3598930`):** Step 20 done — state-driven is now the default install path. `FOX_INSTALL_LEGACY=1` is the escape hatch; the legacy code path is preserved verbatim (deletion held until a few real-install passes confirm no regressions). Step 21 (CLAUDE.md docs) done. Step 22 (this status block + master-plan close-out) done.
- **Steps complete:** 21 / 22 — only Step 19 deferred (no module currently sets `requires_graphical=true`; revisit when one does).
- **Slice complete:** ✅ **Phase 6 closed 2026-05-19.** Pending follow-up: the legacy code-path deletion + the no-graphical-session gate become natural commits once real-install miles accrue.
