...
| 6 ✅  | State-driven installer rebuild       | **Shipped 2026-05-19** (cutover landed at commit `3598930`). State-driven is the default install path; `FOX_INSTALL_LEGACY=1` is the escape hatch. **Key deviation from original brief**: ftxui dropped in favour of pure printf/ANSI to avoid adding a bootstrap-path pacman dep — see subplan §"Wizard + execution". **In scope and shipped**: state manifest with SHA256 source_hash + atomic tmp+rename writes; three-hash classifier; per-module `state_check` callbacks (14 of 52 modules wired so far — coverage grows incrementally); conflict resolution helper with `.foxml-bak` snapshots; interactive ASCII wizard (hjkl, SPACE toggle/cycle, q abort) + manifest preview; repair-mode `--full` semantics; `R17` install lockfile; subprocess error absorption (per-run log under `$XDG_STATE_HOME/foxml/`); `R14` preflight `f_frsize` math fix; `R15` parallel-make + build-time line; `R4a` Firefox install gap verified already ported. **Two pre-existing bugs caught & fixed along the way**: dispatcher's silent `source_hash=""` regression (every re-install would have classified as Conflict); `--resume`/`--phase` dead since the bash port. **Deferred / open**: Step 19 (no-graphical-session gate) is a no-op today since no module declares `requires_graphical=true`; legacy code-path deletion held back until a few real-install miles confirm no regressions. | `plans/refactor/06-state-driven-installer.md` |
