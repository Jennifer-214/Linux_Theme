...
| `fox-health`      | Library (`libfox-health.a`) — health-check primitives (CheckResult, run_all, worst_exit) plus the registered table of system checks (boot path, PAM stack). One source of truth driving `fox sec health`, fox-install preflight, and the post-transaction pacman hook. See `plans/health-checks.md` for the full 60+-item catalog and phased rollout. |
| `fox-sec-health`  | Leaf binary surfacing `libfox-health.a` as `fox sec health`. Output modes: `--quiet` (exit-code only), default (one-line summary), `--verbose` (detail + fix hint per non-pass). Filter via `--only PREFIX,...` / `--exclude PREFIX,...`. |
